...
The first step in enabling SAML authentication is to modify the `local_settings.py` (for Docker: `./custom/local_settings.py`) file of your MediaCMS deployment. Add the following configuration block to enable SAML support, role-based access control (RBAC), and enforce secure communication settings:
...
* Add the following line to `./custom/local_settings.py`:
